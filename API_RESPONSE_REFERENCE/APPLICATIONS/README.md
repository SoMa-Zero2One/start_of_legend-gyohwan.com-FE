...
### 공통 정보
- `SecurityConfig`가 `/v1/applications/**`에 JWT 인증을 요구한다. 컨트롤러는 `@AuthenticationPrincipal` 파라미터를 받지 않지만, 토큰이 없으면 Spring Security가 요청을 401 ProblemDetail로 거절한다.
...
- **성공 (200 OK)**: `ApplicationDetailResponse`(`compare/dto/ApplicationDetailResponse.java:9`)
  ...
  ```
  - `gpa.criteria`는 `Gpa.Criteria#getValue()`에 따라 문자열 `"4.5"`, `"4.3"`, `"4.0"` 중 하나가 내려간다.
  - `language.testType`는 `Language.TestType` 열거형(`TOEFL_IBT`, `TOEFL_ITP`, `IELTS`, `TOEIC`, `HSK`, `JLPT`, `OTHER`) 중 하나다. 점수/등급은 문자열이며 값이 없으면 `null`.
...
> 인증 없이 호출하면 Spring Security가 다음과 같은 ProblemDetail을 내려준다.
> ```json
> {
>   "type": "about:blank",
>   "title": "Unauthorized",
