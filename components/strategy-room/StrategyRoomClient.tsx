...
export default function StrategyRoomClient() {
  ...
  const filteredSlots = useMemo(() => {
    ...
    if (selectedTab === "지원자가 있는 대학") {
      slots = data.slots.filter((slot) => (slot.choiceCount ?? 0) >= 1);
  ...
  const backgroundSlots = useMemo(() => {
    if (!data) return [];
    return data.slots.filter((slot) => (slot.choiceCount ?? 0) >= 1);
  }, [data]);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <Tabs
        ...
        counts={{
          ...
          "지원자가 있는 대학": data.slots.filter((slot) => (slot.choiceCount ?? 0) >= 1).length,
