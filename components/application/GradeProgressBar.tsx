  ...
  return (
    <div className="flex items-center gap-[12px]">
      ...
      <div className="relative h-[8px] flex-1 overflow-hidden rounded-full bg-gray-200">
        <div className="h-full rounded-full bg-[#056DFF]" style={{ width: `${Math.min(percentage, 100)}%` }} />
      </div>

      {/* 점수 표시 */}
      <div className="flex items-baseline gap-[4px]">
        <span className="text-[20px] font-bold text-[#056DFF]">{Number(score.toFixed(2))}</span>
