...
export const userHandlers = [
  ...
  http.post(`${BACKEND_URL}/v1/users/me/gpas`, async ({ request }) => {
    ...
    const isValidCriteria = validCriterias.some(
      (valid) => Math.abs(body.criteria - valid) <= 0.01
    );

    if (!isValidCriteria) {
      return createErrorResponse(
        400,
        '유효하지 않은 학점 기준입니다. 4.0, 4.3, 4.5 중 하나여야 합니다.'
      );
    }
  ...
  http.post(`${BACKEND_URL}/v1/users/me/password`, async ({ request }) => {
    ...
    if (body.currentPassword !== 'password123456') {
      return createErrorResponse(
        400,
        '비밀번호 변경에 실패하였습니다. 현재 비밀번호를 확인해주세요.'
      );
    }
